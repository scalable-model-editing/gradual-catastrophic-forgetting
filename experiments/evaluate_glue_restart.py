...
def main(
    ...
    # must specify where models are stored and must specify previous run to dump metrics into same directory
    ...
    if type(model_name) is str:
        # TO DO: CHANGE INITIAL MODEL
        assert os.path.exists(model_save_location), "Invalid location for models"
        if restart_index is None: # find most recent model stored if not specified
            edits_completed = [
                int(subdir[0].split('_')[-1])
                for subdir in os.walk(model_save_location)
                if subdir[0].split('_')[-1].isnumeric()
            ]
            restart_index = max(edits_completed)
            print(f"Found number of edits to restart from {restart_index}")

        restart_model_path = model_save_location + "/edits_" + str(restart_index) 
        print(f"Instantiating model from {restart_model_path}")
        model = AutoModelForCausalLM.from_pretrained(restart_model_path).cuda()
    ...
    for e, element_index in enumerate(sampled_indices[args.sample_num]):
        if e >= 1200: break
        if e < restart_index: continue # skip until we get to edit n + 1
        ...
        if (e + 1) % args.glue_eval_interval == 0:
        ...
        if (e + 1) % args.model_save_interval == 0:
            ...
            model_save_folder = model_save_location + '/edits_' + str(e + 1)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Edit number to restart from"
    )
